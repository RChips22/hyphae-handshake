...
pub trait HashExt: HashBackend {
    ...
    fn hkdf<'a>(
        &self,
        key: &Self::Hash,
        outputs: impl IntoIterator<Item = &'a mut [u8]>,
        input_key_materials: impl IntoIterator<Item = &'a [u8]>,
        info: &[u8],
    );
}
...
impl<T: HashBackend> HashExt for T {
    fn hmac<'a>(
        ...
        self.hash_into(
            &mut hmac_inner,
            false,
            once(&key_scratch[0..block_size]).chain(inputs.into_iter().map(|x| x)),
        );
        *key_scratch = [0x5Cu8; MAX_BLOCK_SIZE];
        ...
        self.hash_into(
            output,
            false,
            [&key_scratch[0..block_size], self.hash_as_slice(&hmac_inner)],
        );
    }
        ...
        while let Some(output_truncated) = output_iter.next() {
            ...
            if n == 1 {
                ...
                self.hmac(
                    &prk,
                    &mut temp_output,
                    [self.hash_as_slice(&last_output), info, &[n]],
                )
            }

            self.hash_as_mut_slice(&mut last_output)
                .copy_from_slice(self.hash_as_slice(&*temp_output));
            last_output.clone_from(&temp_output);
            let output = self.hash_as_slice(&temp_output);
            if output_truncated.len() > output.len() {
                debug_assert!(
                    false,
                    "truncated hash output cannot be longer than {}",
                    output.len()
                );
                break;
            }
