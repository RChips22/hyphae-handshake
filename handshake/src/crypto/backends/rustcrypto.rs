...
impl AeadBackend for ChaChaPoly {
    ...
    fn header_protection_mask(
        ...
        let nonce: &[u8; HYPHAE_AEAD_NONCE_LEN] =
            sample[4..].try_into().map_err(|_| CryptoError::Internal)?;
...
impl AnyHash {
    fn debug_assert_initialized(&self) {
        debug_assert!(
            !matches!(self.0, RustCryptoHashProtocol::Uninitialized),
            "uninitialized hash backend"
        );
    }
    ...
    fn hash_into<'a>(
        ...
        match self.0 {
            ...
            RustCryptoHashProtocol::Blake2s => {
                let Ok(h32) = (&mut hash[0..32]).try_into() else {
                    debug_assert!(false);
                    return;
                };
