...
#[cfg(feature = "alloc")]
impl<T: HandshakeDriver, B: CryptoBackend, R: Deref<Target = B>> Drop
    for AllocHyphaeHandshake<T, B, R>
{
    fn drop(&mut self) {
...
fn cache_hash_and_zeroize(
    cache: &mut Option<Vec<u8>>,
    remote: &mut Option<Vec<u8>>,
    noise: &mut Box<impl NoiseHandshake>,
) {
    if cache.is_none() {
